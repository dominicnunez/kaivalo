...
describe('auth landing page behavior', () => {
  ...
  it('renders sign-in navigation for unauthenticated visitors', () => {
    ...
    const signInControl = Array.from(document.querySelectorAll('a[href]')).find((anchor) =>
      (anchor.textContent ?? '').toLowerCase().includes('sign in')
    );

    assert.ok(signInControl, 'Expected a sign-in link for unauthenticated users');
    const href = signInControl.getAttribute('href') ?? '';
    assert.ok(href.length > 0, 'Expected a non-empty sign-in target');

    if (href.startsWith('/')) {
      assert.ok(!href.startsWith('//'), 'Expected a same-origin relative sign-in path');
      const target = new URL(href, preview.baseUrl);
      assert.ok(
        trustedPathPrefixes.some((prefix) => target.pathname === prefix || target.pathname.startsWith(`${prefix}/`)),
    ...
    assert.strictEqual(target.protocol, 'https:', 'Expected sign-in target to use https');
    assert.ok(
      target.origin === 'https://api.workos.com' || target.origin === new URL(preview.baseUrl).origin,
  ...
  it('renders auth-unavailable messaging when auth calls fail', async () => {
    const failureResponse = await httpGet(preview.baseUrl, { 'x-kaivalo-test-auth-failure': '1' });
    ...
    try {
      ...
      assert.ok(
        bannerText.includes('Sign-in is temporarily unavailable. Please try again shortly.'),
        'Expected auth-unavailable copy in banner'
      );
      assert.match(bannerText, /\(ref authlayout_[a-f0-9-]+\)/i);

      const unavailableButton = Array.from(failureDocument.querySelectorAll('button')).find((button) =>
        (button.textContent ?? '').toLowerCase().includes('sign in unavailable')
      );
      assert.ok(unavailableButton, 'Expected disabled sign-in button when auth is unavailable');
