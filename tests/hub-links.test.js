...
describe('hub links', () => {
  ...
  it('renders links in the homepage output', () => {
    ...
    assert.ok(anchors.length > 0, 'Expected at least one href in rendered output');
  });
  ...
  it('does not render internal hash links without explicit in-page navigation', () => {
    ...
    assert.strictEqual(hrefs.length, 0, 'Homepage should not expose hash links without anchor navigation UI');
  });

  it('exposes a valid mailto contact link', () => {
    const hrefs = getAnchors(document).map((anchor) => anchor.getAttribute('href') ?? '');
  ...
  it('does not expose empty or javascript links', () => {
    ...
    assert.strictEqual(hrefs.filter((href) => href.trim().toLowerCase().startsWith('javascript:')).length, 0);
  });
});
