...
async function createHubPreview() {
  ...
  for (let attempt = 0; attempt < PREVIEW_PORT_RETRY_COUNT; attempt += 1) {
    ...
    try {
      return await startPreviewProcess(hubDir, reservedPort.port, reservedPort.release);
    } catch (error) {
      lastError = error;

      // Only retry on startup failures likely related to port binding.
      if (!/address already in use|EADDRINUSE/i.test(String(error?.message ?? ''))) {
        throw error;
      }
    }
  }

  throw lastError ?? new Error('Unable to start hub preview server after retrying ports');
}

async function startPreviewProcess(hubDir, previewPort, releasePortReservation) {
  ...
  while (Date.now() < startupDeadline) {
    if (spawnError) {
      await stopServer();
      throw new Error(`Unable to start hub preview server: ${spawnError.message}`);
    }
