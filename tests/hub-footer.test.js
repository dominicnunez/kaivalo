...
describe('apps/hub footer section', () => {
	...
	it('renders footer with copyright year', () => {
		...
		assert.ok(
			footer.textContent?.includes(currentYear),
	...
	it('renders footer navigation links', () => {
		...
		const hrefs = Array.from(footer.querySelectorAll('a[href]')).map(
			(anchor) => anchor.getAttribute('href') ?? ''
		);
		assert.ok(hrefs.length > 0, 'Footer should include at least one link');
		assert.strictEqual(
			hrefs.filter((href) => href.trim() === '').length,
			0,
			'Footer links should not be empty'
		);
	});
});
