...
describe('hub auth and footer controls', () => {
  ...
  it('renders auth controls with visible text labels and icon affordances', () => {
    ...
    const signInLink = controls.find(
      (control) => control.tagName.toLowerCase() === 'a' && /sign in/i.test(control.textContent ?? '')
    );
    const signInUnavailableButton = controls.find(
      (control) => control.tagName.toLowerCase() === 'button' && /sign in unavailable/i.test(control.textContent ?? '')
    );
    ...
    assert.ok(/sign in/i.test(signInControl.textContent ?? ''), 'Expected sign-in control to include visible text');
  ...
  it('renders service cards and contact action with icon affordances', () => {
    ...
    for (const title of expectedServiceTitles) {
      const cardTitle = [...servicesSection.querySelectorAll('h3')].find((heading) =>
      ...
      assert.ok(card, `Expected "${title}" to render inside a service card container`);
      assert.ok(card.querySelector('svg'), `Expected "${title}" card to render an icon`);
    }

    const controls = [...document.querySelectorAll('a, button')];
    const contactLink = controls.find(
      (control) => control.tagName.toLowerCase() === 'a'
