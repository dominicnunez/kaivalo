...
describe('hub services grid', () => {
  ...
  it('renders a services section with structured cards', () => {
    ...
    assert.ok(headings.length >= 1, 'Services section should include service headings');
    assert.ok(descriptions.length >= 1, 'Services section should include service descriptions');
  });

  it('does not render service action links while services are marked as coming soon', () => {
    const servicesSection = document.getElementById('services');
    assert.ok(servicesSection, 'Services section should be present');
    const hrefs = Array.from(servicesSection.querySelectorAll('a[href]')).map((link) => link.getAttribute('href') ?? '');
    assert.strictEqual(hrefs.length, 0, 'Services section should not expose action links before launch');
  });
});
