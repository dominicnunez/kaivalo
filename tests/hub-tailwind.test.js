...
function getStylesheetHref(html) {
  ...
  const hrefs = linkTags
    ...
    .filter(Boolean);
  return hrefs.find((href) => href.startsWith('./_app/') || href.startsWith('/_app/')) ?? null;
}
...
describe('hub styling behavior', () => {
  ...
  it('uses local styling assets and safe link protocols', () => {
    ...
    const hrefMatches = [...homepage.data.matchAll(/\bhref="([^"]+)"/gi)].map((match) => match[1]);
    assert.ok(hrefMatches.length > 0, 'expected at least one link in homepage markup');
    ...
    assert.match(String(stylesheet.headers['content-type'] ?? ''), /^text\/css/i);
    assert.ok(stylesheet.data.length > MIN_STYLESHEET_BYTES, 'generated stylesheet should be non-trivial');
    assert.match(stylesheet.data, /@font-face/i, 'generated stylesheet should include font-face rules');
    assert.match(stylesheet.data, /\/fonts\/clash-display-400\.woff2/i, 'stylesheet should reference local font assets');
  ...
  it('serves local font assets over first-party static routes', async () => {
    const fontResponse = await httpGet(new URL('/fonts/clash-display-400.woff2', stylesheetUrl).toString());
    assert.strictEqual(fontResponse.statusCode, 200);
    assert.match(String(fontResponse.headers['content-type'] ?? ''), /^font\//i);
  });
});
