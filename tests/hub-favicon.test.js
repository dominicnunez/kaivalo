...
test('favicon.ico contains multiple sizes', () => {
	const buffer = readFileSync(faviconIco);
...
test('favicon-192.png exists', () => {
	assert.ok(existsSync(favicon192), 'favicon-192.png should exist');
});

test('favicon-192.png has valid PNG header', () => {
...
test('favicon-512.png exists', () => {
	assert.ok(existsSync(favicon512), 'favicon-512.png should exist');
});

test('favicon-512.png has valid PNG header', () => {
...
// ── app.html integration test ──

test('app.html references SVG favicon', () => {
...
test('app.html references ICO favicon as fallback', () => {
	const appHtml = readFileSync(join(projectRoot, 'apps', 'hub', 'src', 'app.html'), 'utf8');
