...
function assertStaticAssetHeaders(response) {
  ...
  assert.ok(contentLength > 0, 'asset responses should include a positive content length');
  assert.ok(response.headers['last-modified'], 'asset responses should include last-modified');
}
...
function assertCacheControl(response, expected) {
  ...
  assert.strictEqual(normalize(response.headers['cache-control']), normalize(expected));
}
...
describe('hub favicon delivery', () => {
  ...
  it('serves favicon.ico as a static asset', async () => {
    ...
    assertCacheControl(response, 'public, max-age=86400, stale-while-revalidate=600');
  });
  ...
  it('serves web app icons as png images', async () => {
    ...
    assertCacheControl(icon192, 'public, max-age=86400, stale-while-revalidate=600');
    ...
    assertCacheControl(icon512, 'public, max-age=86400, stale-while-revalidate=600');
  });
  ...
  it('serves immutable app assets with immutable cache policy and hardening', async () => {
    ...
    assert.ok(match, 'homepage should reference at least one immutable app asset');
  ...
  it('serves font assets with static hardening and reusable caching', async () => {
    const response = await httpGet(`${preview.baseUrl}/fonts/clash-display-400.woff2`);
    assert.strictEqual(response.statusCode, 200);
    assert.match(String(response.headers['content-type'] ?? ''), /^font\/woff2/);
    assertStaticAssetHeaders(response);
    assertStaticSecurityHeaders(response);
    assertCacheControl(response, 'public, max-age=604800, stale-while-revalidate=86400');
  });
});
