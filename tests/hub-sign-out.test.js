  ...
  it('returns a sanitized 503 with incident reference when upstream signOut fails', async () => {
    ...
    await assert.rejects(
      ...
      (caught) => {
        ...
        assert.match(caught.body.message, /^Sign-out failed\. Reference: authso_/);
        return true;
      }
    );
    ...
    assert.strictEqual(logs[0][1].errorCode, 'AUTH_SIGN_OUT_UNEXPECTED_FAILURE');
    assert.ok(!('errorStack' in logs[0][1]), 'stack traces must not be logged for sign-out failures');
...
describe('sign-out route integration behavior', () => {
  ...
  it('accepts same-origin POST requests at route level', async () => {
    ...
    assert.strictEqual(response.statusCode, 302, `expected 302 redirect, received ${response.statusCode}`);
    assert.strictEqual(response.headers.location, '/', 'sign-out should redirect to the homepage');
