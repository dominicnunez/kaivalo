...
describe('npm run preview smoke', () => {
	...
	it('boots preview and renders homepage route with auth controls', () => {
		...
		assert.notStrictEqual(
			pageTitle,
			'',
			'Page should render a non-empty title'
		);
		assert.ok(
			document.querySelector('main'),
			'Page should render a main landmark'
		);
		assert.ok(
		...
		assert.ok(
			...
			'Page should render a footer landmark'
		);

		const signOutForm = document.querySelector(
			'form[action="/auth/sign-out"][method="POST"]'
		);
		const authLinks = Array.from(document.querySelectorAll('a[href]')).map(
			(anchor) => anchor.getAttribute('href') ?? ''
		);
		const hasSignInLink = authLinks.some(
			(href) =>
				href.includes('/auth/sign-in') ||
				href.includes('/user_management/authorize')
		);
		assert.ok(
			signOutForm || hasSignInLink,
			'Page should expose auth entry/exit controls'
		);
	});
