...
import { getProxyTrustConfiguration, getValidatedWorkosEnv } from '../apps/hub/src/lib/server/workos-security.js';
...
describe('apps/hub/.env.example behavior', () => {
 const envValues = parseEnvTemplate(content);
  ...
  it('documents a local development configuration that validates with real secrets', () => {
    ...
    assert.strictEqual(validated.redirectUri, 'http://localhost:5173/auth/callback');
    assert.strictEqual(validated.origin, 'http://localhost:5173');
    assert.strictEqual(validated.cookiePassword, fixtureSecrets.WORKOS_COOKIE_PASSWORD);
  });
  ...
  it('documents a production example that validates with real secrets', () => {
    ...
    assert.strictEqual(validated.redirectUri, 'https://hub.kaivalo.com/auth/callback');
