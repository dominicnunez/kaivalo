...
describe('hub about section', () => {
  ...
  it('renders a targetable about section within the main landmark', () => {
    ...
    assert.strictEqual(aboutSection.tagName.toLowerCase(), 'section', 'about should be a section landmark');

    const main = document.querySelector('main');
    assert.ok(main, 'main landmark should exist');
    assert.ok(main.contains(aboutSection), 'about section should render within main content');
    ...
    assert.ok((heading.textContent ?? '').trim().length > 0, 'about heading text should be non-empty');
  });
  ...
  it('provides non-empty descriptive text content', () => {
    ...
    assert.ok(paragraphs.length >= 2, 'about section should include descriptive paragraph content');
    for (const paragraph of paragraphs) {
      const text = (paragraph.textContent ?? '').replace(/\s+/g, ' ').trim();
      assert.ok(text.length > 0, 'about section paragraphs should not be empty');
    }
  });
});
