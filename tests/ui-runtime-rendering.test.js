...
describe('ui package runtime behavior', () => {
  ...
  it('renders a valid unauthenticated auth control and a contact control', () => {
    ...
    const signInLink = controls.find(
      (element) => element.tagName.toLowerCase() === 'a' && /sign in/i.test(element.textContent ?? '')
    );
    const signInUnavailableButton = controls.find(
      (element) =>
        element.tagName.toLowerCase() === 'button' && /sign in unavailable/i.test(element.textContent ?? '')
    );
    ...
    if (signInLink) {
      ...
      const trustedHost = parsed.origin === new URL(preview.baseUrl).origin || parsed.origin === 'https://api.workos.com';
      const trustedPath = parsed.pathname.startsWith('/auth/sign-in') || parsed.pathname.startsWith('/user_management/authorize');
    ...
    if (signInUnavailableButton) {
      assert.strictEqual(signInUnavailableButton.getAttribute('type'), 'button');
      assert.strictEqual(signInUnavailableButton.getAttribute('disabled'), '');
      assert.strictEqual(signInUnavailableButton.getAttribute('aria-disabled'), 'true');
    }
    ...
    assert.ok(contactLink, 'Expected a mailto contact control with a visible label');
  });
});
