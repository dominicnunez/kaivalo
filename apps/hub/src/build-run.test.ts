...
describe('hub build runner', () => {
	it('runs svelte-kit sync before vite build and runtime preparation', () => {
	...
	it('reports sync failures before invoking vite build', () => {
		const removeSourceMaps = vi.fn();
		const assertNodeVersion = vi.fn();
		const syncError = new Error('spawnSync pnpm ENOENT');
		...
		expect((thrownError as Error & { cause?: unknown }).cause).toBe(syncError);
	});
