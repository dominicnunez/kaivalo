<script lang="ts">
  ...
  const INCIDENT_ID_PATTERN = /^(auth(?:cb|so|layout)|hook)_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  ...
  function resolveAuthError(
    ...
    const candidate = source ?? (queryAuthError
      ? {
  ...
  let queryIncidentId = $derived(sanitizeIncidentId(page.url.searchParams.get('incident')));
  let authError = $derived(resolveAuthError(data.authError, queryAuthError, queryIncidentId));
