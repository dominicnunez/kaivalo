...
- If tests PASS:
  - Update PRD.md to mark the task complete (change [ ] to [x])
  - Commit your changes with message: [task description] (no conventional commit prefixes, no Co-Authored-By)
  - Append what worked to $PROGRESS_FILE
EOF
)
fi
...
write_prompt_file() {
    local tmpfile
    tmpfile=$(mktemp "${TMPDIR:-/tmp}/ralph-prompt.XXXXXX")
    printf '%s' "$1" > "$tmpfile"
    echo "$tmpfile"
}
...
run_claude() {
    ...
    # Prompt written to file to avoid shell metacharacter injection
    script -q /dev/null -c "claude --model '$CLAUDE_MODEL' --dangerously-skip-permissions -p \"\$(cat '$prompt_file')\""
    rm -f "$prompt_file"
}
...
run_opencode() {
    ...
    # Prompt written to file to avoid shell metacharacter injection
    script -q /dev/null -c "opencode run --model '$CURRENT_OC_PRIME_MODEL' \"\$(cat '$prompt_file')\""
    rm -f "$prompt_file"
}
